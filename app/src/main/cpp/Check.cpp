...
int checkDebugger(JNIEnv *env, jobject obj) {
    ...
    auto pid = fork();
    prctl(PR_SET_DUMPABLE, 1); //改变了所有文件的所有者：/proc/(pid)的位置
    ...
    if (pid != 0) {
        ...
        do {
            ...
            if(childpid > 0) {// pid>0时，只等待进程ID等于pid的子进程，不管其它已经有多少子进程运行结束退出了，只要指定的子进程还没有结束，waitpid就会一直等下去。
                succ = childstatus == 1;
...
jboolean Java_com_sh_antidebugger_check(JNIEnv *env, jobject obj, jstring flag) {
    LOGI("check flag: current encoded flag is %s", sflag);
    ...
    // 在线xor计算  https://gchq.github.io/CyberChef/#recipe=From_Hex('Auto')XOR(%7B'option':'Hex','string':'0x1e'%7D,'Standard',false)To_Hex('Space',0)&input=MHg0QQ
    // xor 性质 任意两个数异或为第三个数，第三个数任意异或其中的一个等于另一个
    char tflag[0x10+1] = {
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    JNIEnv *env = nullptr;
    jint result = -1;
    LOGD("check flag: current encoded flag is 12312312312");
    if (vm->GetEnv((void **) &env, JNI_VERSION_1_6) != JNI_OK) {
        return -1;
    }
    {
        auto clazz1 = env->FindClass("com/sh/antidebugger/App");
        if (clazz1 == nullptr) {
            return -1;
        }
        ...
        if (env->RegisterNatives(clazz1, method, 1) < 0) {
            return -1;
        }
    ...
    {
        auto clazz = env->FindClass("com/sh/antidebugger/MainActivity");
        if (clazz == nullptr) {
            return -1;
        }
        JNINativeMethod method[] = {
                {"check", "(Ljava/lang/String;)Z",(void*) Java_com_sh_antidebugger_check},
                {"sum", "(II)I", (void*)sum},
        };
